#!/usr/bin/env python
"""Run controlled drift experiment on the Taiwan Credit dataset.
...
Usage:
    uv run python experiments/run_taiwan_credit.py
    uv run python experiments/run_taiwan_credit.py --fast   # Quick run (fewer permutations)

Results are saved to results/taiwan_credit_controlled.json
"""
...
logger = logging.getLogger("taiwan_credit_experiment")


def main() -> None:
    parser = argparse.ArgumentParser(description="Taiwan Credit controlled experiment")
    parser.add_argument(
        "--fast",
        action="store_true",
        help="Fast mode: fewer permutations (50 instead of 500)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="results/taiwan_credit_controlled.json",
    ...
    logger.info("=" * 60)
    logger.info("SWIFT Controlled Experiment: Taiwan Credit Dataset")
    logger.info("=" * 60)
    logger.info("Mode: %s (n_permutations=%d, max_samples=%s)",
                "FAST" if args.fast else "FULL", n_permutations,
    ...
    for sr in result.scenario_results:
        ...
        max_psi = max(sr.baseline_scores.get("PSI", {}).values()) if sr.baseline_scores.get("PSI") else 0
        max_ks = max(sr.baseline_scores.get("KS", {}).values()) if sr.baseline_scores.get("KS") else 0
        bbsd_ks = sr.baseline_scores.get("BBSD_KS", {}).get("_model_output", 0)

        print(
            f"  {sr.scenario} (mag={sr.magnitude:.1f}): "
    ...
    print("-" * 70)
    print(f"{'Scenario':<6} {'Mag':>5} {'SWIFT_max':>11} {'SWIFT_mean':>12} {'PSI_max':>9} {'BBSD_KS':>9} {'Detected':>10}")
    print("-" * 65)
    for sr in result.scenario_results:
        max_psi = max(sr.baseline_scores.get("PSI", {}).values()) if sr.baseline_scores.get("PSI") else 0
        bbsd_ks = sr.baseline_scores.get("BBSD_KS", {}).get("_model_output", 0)
        print(
            f"{sr.scenario:<6} {sr.magnitude:>5.1f} {sr.swift_max:>11.6f} "
