#!/usr/bin/env python
"""Run controlled drift experiment on the Home Credit dataset.
...
Home Credit Default Risk (Kaggle) has ~307K rows and ~120 features,
including 16 categorical columns. It has a low default rate (~8%),
making it a challenging dataset for credit scoring models. This
experiment tests SWIFT's scalability to high-dimensional data.

Usage:
    uv run python experiments/run_home_credit.py
    uv run python experiments/run_home_credit.py --fast   # Quick run (fewer permutations)

Results are saved to results/home_credit_controlled.json
"""
...
logger = logging.getLogger("home_credit_experiment")


def main() -> None:
    parser = argparse.ArgumentParser(description="Home Credit controlled experiment")
    parser.add_argument(
        "--fast",
        action="store_true",
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/home_credit_controlled.json",
    ...
    logger.info("=" * 60)
    logger.info("SWIFT Controlled Experiment: Home Credit Dataset")
    logger.info("=" * 60)
    logger.info(
    ...
    t0 = time.time()
    from experiments.data_loader import load_home_credit
    ...
    # Home Credit has both numeric and categorical features, so include S6
    config = ExperimentConfig(
        ...
        alpha=0.05,
        ref_fraction=0.6,
        n_features_to_drift=5,  # More features to drift (120+ feature dataset)
